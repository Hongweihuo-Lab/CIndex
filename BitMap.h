...
# Filename: BitMap.h
# Ver 2.0 2022-06-10
# Copyright (C) 2022 Chenhui Wang, Pengfei Liu, Zongtao He, Hongwei Huo, Jeffrey Scott Vitter
#
...
class Dmap
{
  ...
  public:
	...
	u64 rank2(u64 i)
	{
		...
		u64 Sdnowi = Sbdbefore + Bd->GetValue(Mdnum);
		u64 nextrank;
		if(Mdnum == Bd->Getdatanum() - 2)
			nextrank = Sd->Getdatanum();
		else
			nextrank = Bd->GetValue(Mdnum + 1) + SBd->GetValue((Mdnum + 1) >> SBlength);
		u64 len = nextrank - Sdnowi;
		for(u64 i = 0;i < len;++i)
		{
			u64 Sdnow = Sd->GetValue(Sdnowi);
			if (((Mdnum << Blength) + Sdnow) > i)
